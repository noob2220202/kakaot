...
기본값은 `127.0.0.1`(서버 자기 자신만 접속 가능)입니다. 다른 기기에서 접속하려면
`ecosystem.config.js`의 `HOST: "0.0.0.0",` 주석을 풀고 `npm run restart`를 실행한 뒤
`http://서버IP:7777`로 접속하세요.

> 인증이 없으니 서버 IP에 닿을 수 있는 사람은 누구나 접속할 수 있습니다. 외부 인터넷에
> 노출된 서버라면 방화벽 등으로 접근을 제한하는 걸 권장합니다.
