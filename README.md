...
npm run login       # 브라우저가 열리면 카카오 계정으로 로그인 → 터미널로 돌아와 Enter (세션 저장)
npm run up          # pm2로 서버 기동 + 부팅 목록 저장
```

`npm run login`은 브라우저 로그인이 필요한 **인터랙티브 1회 작업**이라 pm2로 띄우지 않습니다.
로그인이 끝나면 `session.json`이 생성되고, 이후에는 서버가 매 요청마다 이 파일을 읽어
로그인 상태를 재사용합니다.
